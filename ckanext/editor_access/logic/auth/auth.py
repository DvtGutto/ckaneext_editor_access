...
def user_is_member_of_package_org(user, package):
    ...
    if package.owner_org:
        ...
        if role_in_org == 'editor' or role_in_org == 'admin':
            return True
    return False

def user_is_admin_of_package_org(user, package):
    """
    Return True if the package is in an organization and the user has the member role in
    that organization.
...
def user_owns_package(user, package):
    """
    Checks that the given user created the package, and has the 'editor' role in the
    organization that owns the package.

    :param user: A user object
    :param package: A package object
    :returns: True if the user created the package and has the 'editor' role in the
              organization to which package belongs. False otherwise.
    """
    if  user.id == package.creator_user_id and user_is_member_of_package_org(user, package):
        return True
    elif user.id != package.creator_user_id and user_is_admin_of_package_org(user, package):
    	return True
    return False
